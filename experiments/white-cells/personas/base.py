...
Every White Cells persona implements:
  ...
  - `engine` (class attribute) — short label naming the upstream MIT
    engine wrapped, or "stdlib" for hand-built personas
...
PyRIT). The `_load_engine()` method is the import boundary; Phase 2
swaps the stub for the real `import` + adapter without touching the
supervisor or the rest of the persona class.
"""
...
class _StubEngine:
    ...
    The real promptfoo / PyRIT adapter will: (1) configure the engine
    with the persona's prompts/probes, (2) target the fixture sandbox
    URLs, (3) collect each engine's raw findings, (4) translate to the
    v1 schema. Phase 1 skips (1)-(3) and emits canned (4)-shape rows
    so the rest of the supervisor pipeline runs end-to-end.
    """
